...
if current_platform.is_cuda_alike():
    ...
    from .fusion.qwen35_qknorm_rope_kvcache_fusion import (
        Qwen35QkNormRopeKvCacheFusionPass,
    )
...
class PostGradPassManager(CustomGraphPass):  # type: ignore[misc]
    ...
    def configure(self, config: VllmConfig) -> None:
        ...
        with set_current_vllm_config(config, check_compile=False):
            ...
            if self.pass_config.fuse_qwen35_qknorm_rope_kvcache:
                self.passes += [SplitCoalescingPass(config)]
                self.passes += [ScatterSplitReplacementPass(config)]
                self.passes += [Qwen35QkNormRopeKvCacheFusionPass(config)]
