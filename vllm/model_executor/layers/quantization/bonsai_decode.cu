...
__global__ void bonsai_decode_lut_kernel(const uint8_t* __restrict__ packed,
                                         ...
                                         int8_t* __restrict__ out, int64_t n) {
...
static torch::Tensor bonsai_decode_lut(torch::Tensor packed,
                                       torch::Tensor lut) {
  ...
  auto out = at::empty({packed.numel(), 4}, packed.options().dtype(at::kChar));
  ...
  bonsai_decode_lut_kernel<<<blocks, threads, 0, stream>>>(
      packed.data_ptr<uint8_t>(), lut.data_ptr<int8_t>(),
      out.data_ptr<int8_t>(), n);
  C10_CUDA_KERNEL_LAUNCH_CHECK();
  return out;
}

template <typename scalar_t, int TILE_N, int WARPS>
__global__ void bonsai_q2b1_gemm_kernel(const scalar_t* __restrict__ x,
  ...
  for (int i = tid; i < 256 * 4; i += blockDim.x) {
    ...
    smem_lut[byte][slot] =
        static_cast<int8_t>(code == 1 ? 1 : (code == 2 ? -1 : 0));
  }
  __syncthreads();
  ...
  for (int tile_row = warp; tile_row < TILE_N; tile_row += WARPS) {
    ...
    if (row < m && output_row < n) {
      for (int64_t group = 0; group < scale_stride; ++group) {
        float scale = 0.0f;
        if (lane == 0) {
          scale = static_cast<float>(scales[output_row * scale_stride + group]);
        }
...
static void launch_bonsai_q2b1_gemm_variant(torch::Tensor x,
  ...
  bonsai_q2b1_gemm_kernel<scalar_t, TILE_N, WARPS><<<grid, block, 0, stream>>>(
      x.data_ptr<scalar_t>(), packed.data_ptr<uint8_t>(),
      scales.data_ptr<at::Half>(), out.data_ptr<at::BFloat16>(), m, n, k);
}

template <typename scalar_t>
static void launch_bonsai_q2b1_gemm(torch::Tensor x, torch::Tensor packed,
  ...
  switch (config_id) {
    case 0:
      launch_bonsai_q2b1_gemm_variant<scalar_t, 64, 2>(x, packed, scales, out);
      break;
    case 1:
      launch_bonsai_q2b1_gemm_variant<scalar_t, 128, 4>(x, packed, scales, out);
      break;
    case 2:
      launch_bonsai_q2b1_gemm_variant<scalar_t, 256, 8>(x, packed, scales, out);
      break;
    default:
...
static torch::Tensor bonsai_q2b1_gemm(torch::Tensor x, torch::Tensor packed,
                                      torch::Tensor scales, int64_t config_id) {
  TORCH_CHECK(config_id >= 0 && config_id <= 2, "config_id must be 0, 1, or 2");
  ...
  TORCH_CHECK(
      x.is_contiguous() && packed.is_contiguous() && scales.is_contiguous(),
  ...
  TORCH_CHECK(packed.size(1) == k / 4, "packed must have shape (N, K // 4)");
  ...
  AT_DISPATCH_SWITCH(x.scalar_type(), "bonsai_q2b1_gemm",
                     AT_DISPATCH_CASE(at::ScalarType::Float, [&] {
                       launch_bonsai_q2b1_gemm<float>(x, packed, scales, out,
                                                      config_id);
                     }) AT_DISPATCH_CASE(at::ScalarType::BFloat16, [&] {
                       launch_bonsai_q2b1_gemm<at::BFloat16>(x, packed, scales,
                                                             out, config_id);
                     }));
  return out;
}
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
  m.def("bonsai_decode_lut", &bonsai_decode_lut,
        "Q2b1 bytes -> trits via 256x4 LUT");
