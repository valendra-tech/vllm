...
def test_fwht_signs_cpu_preserves_leading_dimensions():
    torch.manual_seed(2)
    ...
    y = fwht_signs(x, signs)
    ref = fwht_signs(x.reshape(-1, 1024), signs).reshape_as(x)

    assert y.shape == x.shape
    torch.testing.assert_close(y, ref)


def test_fwht_fp8_cpu_preserves_leading_dimensions():
    torch.manual_seed(3)
    x = torch.randn(2, 3, 1024)
    signs = torch.tensor([-1.0, 1.0] * 512)
    from vllm.model_executor.layers.quantization.hadamard_fwht import (
        fwht_signs_quant_fp8,
    )

    q, amax = fwht_signs_quant_fp8(x, signs, group=128)

    assert q.shape == (2, 3, 1024)
    assert amax.shape == (2, 3, 8)
    assert q.dtype == torch.float8_e4m3fn
    assert amax.dtype == torch.float32


def test_fwht_signs_dispatches_to_triton(monkeypatch):
    if not torch.cuda.is_available():
        pytest.skip("cuda required")
    import vllm.model_executor.layers.quantization.hadamard_fwht as hadamard_fwht

    if not hadamard_fwht._HAS_TRITON:
        pytest.skip("triton required")

    expected = torch.empty((1, 1024), dtype=torch.float32, device="cuda")
    monkeypatch.setattr(
        hadamard_fwht,
        "fwht_signs_triton",
        lambda x, signs: expected,
        raising=True,
    )
    x = torch.empty((1, 1024), dtype=torch.float32, device="cuda")
    signs = torch.ones(1024, device="cuda")

    assert hadamard_fwht.fwht_signs(x, signs) is expected


def test_fwht_signs_triton_handles_strided_rows():
    if not torch.cuda.is_available():
        pytest.skip("cuda required")
    import vllm.model_executor.layers.quantization.hadamard_fwht as hadamard_fwht
    ...
    torch.manual_seed(4)
    full = torch.randn(3, 2048, dtype=torch.float32, device="cuda")
    x = full[:, :1024]
    signs = torch.tensor([-1.0, 1.0] * 512, device="cuda")
    y = hadamard_fwht.fwht_signs(x, signs)
    H = torch.from_numpy(hadamard_matrix(1024)).to("cuda")
    ref = (x * signs) @ H.T
